...
import com.nordstrom.nlp.NlpConfig;
...
public class HealthCheckAppenderProxyRouteConfigFactoryTest extends Assert {
  HealthCheckAppenderProxyRouteConfigFactory subject;
  ...
  public void testBuild() {
    ...
    ProxyRouteConfig healthCheck = results.get(3);
    ...
    DynamicRouteConfig healthCheckRouteConfig =
        healthCheckRouteConfigFactory.buildHealthCheckRouteConfig();
    assertEquals(healthCheckRouteConfig.getPath(), healthCheck.path());
    assertEquals(
        healthCheckRouteConfig.getClientConfigs().get(0).getIpAddress(),
        healthCheck.clientConfigs().get(0).remote().getHostString());
    assertEquals(
        healthCheckRouteConfig.getClientConfigs().get(0).getPort(),
        healthCheck.clientConfigs().get(0).remote().getPort());
    assertEquals(
        healthCheckRouteConfig.getClientConfigs().get(0).isTlsEnabled(),
        healthCheck.clientConfigs().get(0).isTlsEnabled());
  }
}
